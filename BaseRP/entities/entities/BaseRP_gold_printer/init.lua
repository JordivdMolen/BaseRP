...
--Settings--
local Color = Color( 255, 208, 0, 255 )
local PrintTime1 = 50 --Range of the random timer--
local PrintTime2 = 250
local Health = 1000
local PrintAmountRange1 = 1000
local PrintAmountRange2 = 1500
local overheatchance = 10 -- 3 is minimum and biggest change to explode
local PrintMore
...
function ENT:Initialize()
	...
	self:Getowning_ent():SetNWInt("GoldP", (self:Getowning_ent():GetNWInt("GoldP") or 0 ) + 1)
	DarkRP.notify(self:Getowning_ent(), 0, 5, "you own now: " ..self:Getowning_ent():GetNWInt("GoldP") .." printers of this kind")
	if self:Getowning_ent():GetNWInt("GoldP") == Limit then
	self:Remove()
...
function ENT:OnTakeDamage(dmg)
	...
	DarkRP.notify(self:Getowning_ent(), 0, 7, tostring(dmg:GetAttacker()) .." did destroy your printer, !!DESTROYER DID RECEIVE 1/2 OF BUY PRICE")
	  dmg:GetAttacker():addMoney(12500)
...
function ENT:CreateMoneybag()
	...
	self.sparking = false
	timer.Simple(math.random(PrintTime1, PrintTime2), function() PrintMore(self) end)
	DarkRP.notify(self:Getowning_ent(), 0, 7, "your Gold printer has printed: $" ..amount)
end
...
function ENT:OnRemove()
	...
	self:Getowning_ent():SetNWInt("GoldP", (self:Getowning_ent():GetNWInt("GoldP") or 0 ) - 1)
	DarkRP.notify(self:Getowning_ent(), 0, 5, "your moneyprinters is sucesfully removed. it has been deleted from the limit count. you own now: " ..self:Getowning_ent():GetNWInt("GoldP") .." printers of this kind")
end