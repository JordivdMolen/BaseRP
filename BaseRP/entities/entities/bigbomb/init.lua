...
function ENT:Use(activator,caller)
	...
	if (self.Armed) then
		if (self.LastUsed+0.3>CurTime() && self.Disarming==60) then
			...
			self.LastUsed = CurTime()+0.1
			self.Disarming = (self.Disarming or  60) -1
			...
			if (self.Disarming <= 0) then
				DarkRP.notify(activator,1,3, "Bomb defused!")
				DarkRP.notify(self.Owner,1,3, "Bomb has been defused.")
...
function ENT:Armbomb(planter)
	...
	if (self.Entity:CheckPropFaggotry()==false) then
		...
		for k, v in pairs(player.GetAll()) do
			...
			if (propnum==nil) then propnum = 0 end
			if (tonumber(propnum)>0) then
				DarkRP.notify(player.GetByUniqueID(uid), 1, 3, "A bigbomb has been planted near " .. tostring(owners[uid]) .. " of your props!")
		...
		DarkRP.Notify(planter, 1, 3, "Bomb has been planted.")
	else
		self.Arming = 50
		DarkRP.notify(self.Owner, 4, 3, "Get your props away from the bomb for it to be usable.")
		DarkRP.notify(planter, 4, 3, "bomb owner's props are too close.")
	end
end
