...
--Settings--
local Color = Color( 181, 181, 181, 255 )
local PrintTime1 = 100 --Range of the random timer--
local PrintTime2 = 300
local Health = 500
local PrintAmountRange1 = 500
local PrintAmountRange2 = 1000
local overheatchance = 50 -- 3 is minimum and biggest change to explode
local PrintMore
...
function ENT:Initialize()
	...
	self:Getowning_ent():SetNWInt("SilverP", (self:Getowning_ent():GetNWInt("SilverP") or 1 ) + 1)
	DarkRP.notify(self:Getowning_ent(), 0, 5, "you own now: " ..self:Getowning_ent():GetNWInt("SilverP") .." printers of this kind")
	if self:Getowning_ent():GetNWInt("SilverP") >= Limit then
	self:Remove()
...
function ENT:CreateMoneybag()
	...
	self.sparking = false
	timer.Simple(math.random(PrintTime1, PrintTime2), function() PrintMore(self) end)
	DarkRP.notify(self:Getowning_ent(), 0, 7, "your Silver printer has printed: $" ..amount)
end
...
function ENT:OnRemove()
	...
	self:Getowning_ent():SetNWInt("SilverP", (self:Getowning_ent():GetNWInt("SilverP") or 0 ) - 1)
	DarkRP.notify(self:Getowning_ent(), 0, 5, "your moneyprinters is sucesfully removed. it has been deleted from the limit count. you own now: " ..self:Getowning_ent():GetNWInt("SilverP") .." printers of this kind")
end